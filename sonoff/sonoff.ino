...
{
  //niels added
  //save turn off time when turned off
  {
    uint8_t device = 1;
    ...
    if(rpower == sysPower && is_on()){
      //switch will be turned off and is on
      //=>save millis
      last_time_turned_off = millis();
    }
  }
  //publish to white state when turned on
  {
    uint8_t device = 1;
    byte mask = 0x01 << (device -1);
    uint8_t sysPower = sysCfg.power;
    ...
    char svalue[128];  // was MESSZ
      ...
      //=>save millis
      last_time_turned_on = millis();

      unsigned long time_off = millis() - last_time_turned_off;
...
//  if (LOG_LEVEL_DEBUG_MORE <= seriallog_level) Serial.println(dataBuf);
  ...
  if (type != NULL) {
    ...
    if (!strcmp_P(dataBufUc,PSTR("COLD"))) {
      payload = 238;
    }
    if (!strcmp_P(dataBufUc,PSTR("WARM"))) {
      payload = 370;
    }
...
boolean is_on(){
  return last_time_turned_on > last_time_turned_off;
}
